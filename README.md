...
team/
  ...
  <person>/                — Per-person folder
    ...
    activity/              — Per-repo session logs (auto-generated by session-end hook)
      activity-<repo>.md   — Shared logs, one per repo you opted into sharing
      private/             — Everything else. Gitignored, never leaves your machine
projects/
...
| `activity/` | Automatic per-repo session logs (`activity-<repo>.md`) for repos you chose to share | Plugin (session-end hook) |
| `activity/private/` | Every other repo's logs — gitignored, local-only | Plugin (session-end hook) |

### Sharing Is Opt-In

By default a session's log goes to `team/<you>/activity/private/`, which this repo gitignores. It never
gets committed, `/aggregate` and `/retro` never read it, and teammates never see it.

A repo becomes visible to the team only when you say so — add it to `routes` in
`~/.claude-office/identity.json`, mapping its path to `""` (the shared `activity/` root) or to any
subfolder name:
...
```json
{
  ...
  "routes": {
    "~/Documents/GitHub/our-product": "",
    "~/clients": "clients"
  }
}
```

Longest path prefix wins, so subfolders inherit their parent's route. Any subfolder can be added to
`.gitignore` for a second local-only tier — `private/` is just the one that ships pre-ignored.
...
- Activity sharing is opt-in — unrouted repos log to the gitignored `team/*/activity/private/`, so a client or personal repo can't leak by forgetting to blacklist it
